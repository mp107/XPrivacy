...
public class ActivityApp extends ActivityBase {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Display app name
		TextView tvAppName = findViewById(R.id.tvApp);
		...
		// Display app icon
		final ImageView imgIcon = findViewById(R.id.imgIcon);
		...
		final ImageView imgCbOnDemand = findViewById(R.id.imgCbOnDemand);
		...
		swEnabled = findViewById(R.id.swEnable);
		...
		if (!mAppInfo.hasInternet(this)) {
			ImageView imgInternet = findViewById(R.id.imgInternet);
			imgInternet.setVisibility(View.INVISIBLE);
		}

		// Check if frozen
		if (!mAppInfo.isFrozen(this)) {
			ImageView imgFrozen = findViewById(R.id.imgFrozen);
		...
		final ExpandableListView elvRestriction = findViewById(R.id.elvRestriction);
	...
	protected void onNewIntent(Intent intent) {
		...
		if (extras != null && extras.containsKey(cAction) && extras.getInt(cAction) == cActionRefresh) {
			...
			ImageView imgCbOnDemand = findViewById(R.id.imgCbOnDemand);
			boolean ondemand = PrivacyManager
					.getSettingBool(-mAppInfo.getUid(), PrivacyManager.cSettingOnDemand, false);
			imgCbOnDemand.setImageBitmap(ondemand ? getOnDemandCheckBox() : getOffCheckBox());
	...
	private class ApplicationsTask extends AsyncTask<Object, Object, Object> {
		...
		protected Object doInBackground(Object... params) {
			// Get applications
			int count = 0;
			Map<String, List<String>> mapApp = new HashMap<>();
			...
			// Sort applications
			List<String> listApp = new ArrayList<>(mapApp.keySet());
	...
	private class ContactsTask extends AsyncTask<Integer, Object, Object> {
		...
		protected Object doInBackground(Integer... params) {
			// Map contacts
			Map<Long, String> mapContact = new LinkedHashMap<>();
			Cursor cursor;
			if (params[0] < 0)
	...
	private class RestrictionAdapter extends BaseExpandableListAdapter {
		...
		private class GroupViewHolder {
			...
			public GroupViewHolder(View theRow, int thePosition) {
				...
				imgWhitelist = row.findViewById(R.id.imgWhitelist);
		...
		public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
			...
			holder.imgInfo.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View view) {
					...
					// Set info
					TextView tvInfo = dlgHelp.findViewById(R.id.tvInfo);
		...
		private List<Hook> getHooks(int groupPosition) {
			if (!mHook.containsKey(groupPosition)) {
				...
				boolean fUsed = PrivacyManager.getSettingBool(userId, PrivacyManager.cSettingFUsed, false);
				boolean fPermission = PrivacyManager.getSettingBool(userId, PrivacyManager.cSettingFPermission, false);
				List<Hook> listMethod = new ArrayList<>();
		...
		private class ChildViewHolder {
			...
			private ChildViewHolder(View theRow, int gPosition, int cPosition) {
				...
				imgGranted = row.findViewById(R.id.imgGranted);
				imgInfo = row.findViewById(R.id.imgInfo);
				tvMethodName = row.findViewById(R.id.tvMethodName);
				imgUnsafe = row.findViewById(R.id.imgUnsafe);
				imgMethodWhitelist = row.findViewById(R.id.imgMethodWhitelist);
				imgCbMethodRestricted = row.findViewById(R.id.imgCbMethodRestricted);
				pbRunning = row.findViewById(R.id.pbRunning);
				imgCbMethodAsk = row.findViewById(R.id.imgCbMethodAsk);
				llMethodName = row.findViewById(R.id.llMethodName);
			}
		}
	...
	public static void showHelp(ActivityBase context, View parent, Hook hook) {
		...
		// Set title
		TextView tvTitle = dlgHelp.findViewById(R.id.tvTitle);
		...
		if (permissions != null && permissions.length > 0)
			if (!permissions[0].equals("")) {
				TextView tvPermissions = dlgHelp.findViewById(R.id.tvPermissions);
