...
public class PrivacyManager {
	...
	public static List<String> cMethodNoState = Arrays.asList("IntentFirewall", "checkPermission",
			"checkUidPermission");
	...
	private static Map<String, Map<String, Hook>> mMethod = new LinkedHashMap<>();
	private static Map<String, List<String>> mRestart = new LinkedHashMap<>();
	private static Map<String, List<Hook>> mPermission = new LinkedHashMap<>();
	private static Map<CSetting, CSetting> mSettingsCache = new HashMap<>();
	private static Map<CSetting, CSetting> mTransientCache = new HashMap<>();
	private static Map<CRestriction, CRestriction> mRestrictionCache = new HashMap<>();
	private static SparseArray<Map<String, Boolean>> mPermissionRestrictionCache = new SparseArray<>();
	private static SparseArray<Map<Hook, Boolean>> mPermissionHookCache = new SparseArray<>();
	...
	public static List<String> getRestrictions() {
		List<String> listRestriction = new ArrayList<>(Arrays.asList(cRestrictionNames));
		if (Hook.isAOSP(19))
	...
	public static TreeMap<String, String> getRestrictions(Context context) {
		...
		TreeMap<String, String> tmRestriction = new TreeMap<>(collator);
	...
	public static void setRestriction(int uid, String restrictionName, String methodName, boolean restricted,
		...
		List<String> listRestriction = new ArrayList<>();
		if (restrictionName == null)
		...
		List<PRestriction> listPRestriction = new ArrayList<>();
	...
	public static List<String> cIDCant = Arrays.asList("getString", "Srv_Android_ID", "%serialno",
			"SERIAL");
	...
	public static boolean canRestrict(int uid, int xuid, String restrictionName, String methodName, boolean system) {
		...
		if (_uid == Process.SYSTEM_UID) {
			if (PrivacyManager.cIdentification.equals(restrictionName))
				return false;
			if (PrivacyManager.cShell.equals(restrictionName) &&
					methodName != null && methodName.startsWith("loadLibrary"))
				return false;
		}
	...
	public static List<Boolean> getRestartStates(int uid, String restrictionName) {
		...
		List<Boolean> listRestartRestriction = new ArrayList<>();

		Set<String> listRestriction = new HashSet<>();
		if (restrictionName == null)
	...
	public static Map<String, TreeMap<String, Boolean>> listWhitelisted(int uid, String type) {
		...
		Map<String, TreeMap<String, Boolean>> mapWhitelisted = new HashMap<>();
	...
	public static List<PRestriction> getUsageList(Context context, int uid, String restrictionName) {
		...
		List<PRestriction> listUsage = new ArrayList<>();
		try {
	...
	private static final List<String> cSettingAppSpecific = Arrays.asList(cSettingRandom,
	...
	public static Object getDefacedProp(int uid, String name) {
		...
		if (name.equals("InetAddress")) {
			...
			try {
				...
				return unspecified.get(Inet4Address.class);
			} catch (Throwable ex) {
				Util.bug(null, ex);
				return null;
			}
